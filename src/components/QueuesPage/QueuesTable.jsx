...
function Tooltip({ children, content, items = [] }) {
  ...
  const tooltipContent =
    content || (items.length > 0 ? items.join(", ") : null);
              ...
              {items.length > 0 ? (
                <ul className="queues-table__tooltip-list">
                  {items.map((item, index) => (
                    <li key={index} className="queues-table__tooltip-item">
                      {item}
                    </li>
                  ))}
                </ul>
...
function CloneIcon({ className }) {
  return (
    ...
    >
      ...
      />
      <path
        d="M10.5 5.5V3C10.5 2.17157 9.82843 1.5 9 1.5H3C2.17157 1.5 1.5 2.17157 1.5 3V9C1.5 9.82843 2.17157 10.5 3 10.5H5.5"
        stroke="currentColor"
        strokeWidth="1.5"
      />
    </svg>
...
function SubqueueCell({ queue }) {
  ...
  const hasSubqueues =
    queue.distributeSubqueues && queue.subqueues && queue.subqueues.length > 0;
  ...
  const tooltipItems = queue.subqueues
    .slice(1)
    .map((sq) => `${sq.name}: ${sq.percentage}%`);
...
function PriorityCell({ queue }) {
  ...
  if (hasSubqueues) {
    ...
    tooltipItems = queue.subqueues
      .slice(1)
      .map((sq) => `${sq.name}: Priority ${sq.priority}`);
  } else {
...
function PrimaryGroupsCell({ queue }) {
  ...
  if (hasSubqueues) {
    ...
    tooltipItems = queue.subqueues
      .slice(1)
      .map((sq) => `${sq.name}: ${sq.primaryGroups?.join(", ") || "-"}`);
  } else {
    tooltipItems = items.length > 1 ? items.slice(1) : [];
  }
...
function SecondaryGroupsCell({ queue }) {
  ...
  if (hasSubqueues) {
    ...
    tooltipItems = queue.subqueues
      .slice(1)
      .map((sq) => `${sq.name}: ${sq.secondaryGroups?.join(", ") || "-"}`);
  } else {
    tooltipItems = items.length > 1 ? items.slice(1) : [];
  }
...
function BarChartIcon({ className }) {
  return (
    <img
      className={className}
      src={assetUrl(
        "/assets/Bar_chart_square-3f73cdda-5fa1-4ad8-a82a-593e84e09a2b.png",
      )}
      alt=""
      width="12"
      height="12"
          ...
          >
            <div className="queues-table__predictive-tooltip__card">
              <div className="queues-table__predictive-tooltip__content">
                {heading && (
                  <p className="queues-table__predictive-tooltip__heading">
                    {heading}
                  </p>
                )}
                {items.length > 0 && (
                  <ul className="queues-table__predictive-tooltip__list">
                    {items.map((item, i) => (
                      <li
                        key={i}
                        className="queues-table__predictive-tooltip__item"
                      >
                        {item}
                      </li>
                    ))}
...
function PredictiveRoutingCell({
  queue,
  ...
  if (isPredictiveEnabled) {
    ...
    const subqueuesOn = hasMultipleSubqueues
      ? queue.subqueues.filter(
          (sq) => sq.assignmentMethod === "Predictive routing",
        )
      : [];
...
export default function QueuesTable({
  ...
  const handleDragStart = (e, index) => {
    ...
    setTimeout(() => {
      e.target
        .closest(".queues-table__row")
        ?.classList.add("queues-table__row--dragging");
    }, 0);
  };
  ...
  const handleDragEnd = (e) => {
    e.target
      .closest(".queues-table__row")
      ?.classList.remove("queues-table__row--dragging");
        ...
        {queues.map((queue, index) => (
          ...
          >
            <div className="queues-table__cell queues-table__cell--order">
              <div
                className="queues-table__drag-handle"
            ...
            <div className="queues-table__cell queues-table__cell--actions">
              <div
                className="queues-table__actions-container"
                ref={openMenuQueueId === queue.id ? menuRef : null}
              >
                <button
