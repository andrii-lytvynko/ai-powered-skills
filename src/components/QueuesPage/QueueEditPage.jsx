...
import { Anchor, Button, Tag as GardenTag } from "@zendesk-ui/react-components";

import { ChevronDownIcon, SearchIcon, PlusIcon, CheckIcon } from "../Icons";
...
function HelpIcon({ className }) {
  return (
    ...
    >
      ...
      <path
        d="M10 2.5V4.5M10 15.5V17.5M17.5 10H15.5M4.5 10H2.5"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
      />
...
function MetricsCard({ type, data }) {
  ...
  const getConfidenceColorClass = (level) => {
    switch (level) {
      case "high":
        return "predictive-routing__metric-value--success";
      case "medium":
        return "predictive-routing__metric-value--warning";
      case "low":
      case "insufficient":
        return "predictive-routing__metric-value--danger";
      default:
        return "predictive-routing__metric-value--success";
    }
  };
      ...
      <div className="predictive-routing__metric-content">
        <div className="predictive-routing__metric-value-row">
          ...
          {isConfidence && (
            <>
              <span
                className={`predictive-routing__metric-value ${getConfidenceColorClass(data.confidence.level)}`}
              >
...
function PredictiveRoutingAnalyzing({
  queueId,
  queueName,
  progress,
  setProgress,
  ...
  const renderStepText = (step, status) => {
    const textClass =
      status === "pending" ? "predictive-routing__step-text--pending" : "";
    ...
    <div className="predictive-routing__analyzing">
      ...
      <div className="predictive-routing__analyzing-header">
        ...
        <div className="predictive-routing__analyzing-header-content">
          <h3 className="predictive-routing__analyzing-title">
            Evaluating {queueName} queue...
          </h3>
      ...
      <div className="predictive-routing__analyzing-card">
        <div className="predictive-routing__analyzing-card-header">
          ...
          <span className="predictive-routing__analyzing-card-title">
        ...
        <div className="predictive-routing__progress-section">
          ...
          >
            <ChevronUpIcon
              className={`predictive-routing__toggle-icon ${!isExpanded ? "predictive-routing__toggle-icon--collapsed" : ""}`}
            />
            ...
            <div className="predictive-routing__steps">
              {getEvaluationSteps(queueId).map((step, index) => {
                ...
                return (
                  <div
                    key={step.id}
                    className={`predictive-routing__step predictive-routing__step--${status}`}
                  >
...
function PredictiveRoutingResults({
  queueName,
  results,
  onEnablePredictive,
  onDismiss,
  ...
  return (
    <div className="predictive-routing__results">
      ...
      <div className="predictive-routing__results-header">
        <h3 className="predictive-routing__results-title">
          Evaluation results
        </h3>
        <p className="predictive-routing__results-subtitle">
          Based on your <strong>{queueName}</strong> queue data, Predictive
      ...
      <div className="predictive-routing__results-actions">
        <Button isPrimary onClick={onEnablePredictive}>
          Turn on Predictive routing
        </Button>
        <Button isBasic onClick={onDismiss}>
          Dismiss results
        </Button>
      </div>
    </div>
...
function PredictiveRoutingSection({
  isPredictiveEnabled,
  onTogglePredictive,
  hideToggle,
  queueName,
    ...
    <section className="queue-edit-section">
      <div className="queue-edit-section__header">
        ...
        <p className="queue-edit-section__description">
          Route your messaging tickets faster with AI-powered ticket
          distribution.{" "}
      ...
      <div
        className={`predictive-routing-card ${isPredictiveEnabled ? "predictive-routing-card--expanded" : ""}`}
      >
          ...
          <div className="predictive-routing-card__content">
            <div className="predictive-routing-card__header">
              <h3 className="predictive-routing-card__title">
                ...
                <GardenTag
                  size="medium"
                  hue={isPredictiveEnabled ? "green" : "grey"}
                >
        ...
        {isPredictiveEnabled && (
          <div className="predictive-routing-card__expanded-content">
            <div className="predictive-routing-card__section">
              <h4 className="predictive-routing-card__section-title">
                Data dashboards
              </h4>
    ...
    <div className="queue-edit__multiselect" ref={dropdownRef}>
      ...
      >
        <div className="queue-edit__multiselect-tags">
          {value.length === 0 && (
            <span className="queue-edit__multiselect-placeholder">
        ...
        <ChevronDownIcon
          className={`queue-edit__multiselect-chevron ${isOpen ? "queue-edit__multiselect-chevron--open" : ""}`}
        />
      </div>
        ...
        <div className="queue-edit__multiselect-dropdown">
          ...
          <div className="queue-edit__multiselect-options">
            {filteredGroups.length === 0 && (
              <div className="queue-edit__multiselect-empty">
                No groups found
              </div>
            )}
            {filteredGroups.map((group) => (
              ...
              >
                <span className="queue-edit__multiselect-option-text">
                  {group}
                </span>
  ...
  return (
    <div className="queue-edit__condition-row">
      <div className="queue-edit__condition-field">
        <span className="queue-edit__condition-field-text">
          {condition.field || "Ticket > Tags"}
        </span>
      </div>
      <div className="queue-edit__condition-field">
        <span className="queue-edit__condition-field-text">
          {condition.operator || "Contains at least one of the following"}
        </span>
      </div>
      <div className="queue-edit__condition-field">
        <span className="queue-edit__condition-field-text queue-edit__condition-field-text--placeholder">
          {condition.value || ""}
        </span>
      </div>
      <button
        type="button"
        className="queue-edit__condition-remove"
        onClick={onRemove}
    ...
    <div className="subqueue-row" ref={rowRef}>
      ...
      <div className="subqueue-row__field subqueue-row__field--percentage">
        <div className="subqueue-row__input-group">
          <input
            ...
            onChange={(e) =>
              onUpdate(subqueue.id, "percentage", parseInt(e.target.value) || 0)
            }
            min="0"
            max="100"
          />
      ...
      <div className="subqueue-row__field subqueue-row__field--priority">
        <input
          ...
          onChange={(e) =>
            onUpdate(subqueue.id, "priority", parseInt(e.target.value) || 1)
          }
          min="1"
          max="100"
        />
        ...
        <div className="subqueue-row__multiselect">
          <div className="subqueue-row__multiselect-tags">
            ...
            {subqueue.primaryGroups.length > 1 && (
              <span className="subqueue-row__more-link">
                +{subqueue.primaryGroups.length - 1} more
              </span>
            )}
          </div>
        ...
        <div className="subqueue-row__multiselect">
          <div className="subqueue-row__multiselect-tags">
            ...
            {subqueue.secondaryGroups.length > 1 && (
              <span className="subqueue-row__more-link">
                +{subqueue.secondaryGroups.length - 1} more
              </span>
            )}
          </div>
          <ChevronDownIcon className="subqueue-row__multiselect-chevron" />
      ...
      <div className="subqueue-row__field subqueue-row__field--predictive">
        <button
          ...
          aria-label={
            subqueue.assignmentMethod === "Predictive routing"
              ? "Turn off Predictive routing for this subqueue"
              : "Turn on Predictive routing for this subqueue"
          }
        >
          <span className="toggle-switch__slider"></span>
        </button>
...
function SubqueueTable({
  ...
  return (
    <div className="subqueue-table">
      ...
      {!canAddMore && (
        <span className="subqueue-limit-text">
          Maximum of {maxSubqueues} subqueues reached.
        </span>
      )}
    </div>
  );
...
export default function QueueEditPage({
  ...
  const [conditionsAll, setConditionsAll] = useState(
    queue?.conditions?.all || [
      {
        field: "Ticket > Tags",
        operator: "Contains at least one of the following",
        value: "",
      },
    ],
  );
  const [conditionsAny, setConditionsAny] = useState(
    queue?.conditions?.any || [],
  );
  const [distributeSubqueues, setDistributeSubqueues] = useState(
    queue?.distributeSubqueues || false,
  );
  ...
  const updateSubqueue = (id, field, value) => {
    setSubqueues(
      subqueues.map((sq) => (sq.id === id ? { ...sq, [field]: value } : sq)),
    );
  };

  // Calculate total percentage
  const totalPercentage = subqueues.reduce(
    (sum, sq) => sum + (parseInt(sq.percentage) || 0),
    0,
  );
  ...
  const handleSave = () => {
    const updatedQueue = {
      ...
      secondaryGroups: {
        ...queue?.secondaryGroups,
        items: enableSecondaryGroups ? secondaryGroups : [],
      },
      assignmentMethod,
    };
    onSave(updatedQueue);
          ...
          <main className="queue-edit-page__main">
            <div className="queue-edit-page__content">
              ...
              <div className="queue-edit-breadcrumbs">
                <button
                  type="button"
                  className="queue-edit-breadcrumbs__link"
                  onClick={onBack}
                >
                  Objects and rules
                </button>
                ...
                <ChevronDownIcon className="queue-edit-breadcrumbs__separator" />
                <span className="queue-edit-breadcrumbs__current">
                  {isCreating ? "New queue" : queue?.name || "Queue"}
                </span>
              </div>

              {/* Page Header */}
              <div className="queue-edit-page-header">
                <h1 className="queue-edit-page-header__title">
                  {isCreating ? "Create queue" : queue?.name || "Queue"}
                </h1>
              ...
              <section className="queue-edit-section">
                <div className="queue-edit-form">
                  ...
                  <div className="queue-edit-form__field">
                    <label className="queue-edit-form__label">
                      Description
                    </label>
                    <input
                      type="text"
              ...
              {!distributeSubqueues && (
                <section className="queue-edit-section">
                  <div className="queue-edit-section__header">
                    ...
                    <p className="queue-edit-section__description">
                      If a group receives work from more than one queue, work
                      from the queue with the higher priority gets assigned
                      first.
                    </p>
                  </div>
                  ...
                  <div className="queue-edit-form">
                    <div className="queue-edit-form__field queue-edit-form__field--narrow">
                      ...
                      <span className="queue-edit-form__hint">
                        Enter a value between 1–100, where 1 is the highest
                        priority.
                      </span>
                      <input
                        type="text"
              ...
              <section className="queue-edit-section">
                ...
                <div className="queue-edit-conditions">
                  ...
                  <Button
                    className="queue-edit-conditions__add"
                    onClick={addConditionAll}
                  >
                    Add condition
                  </Button>
                </div>
              ...
              <div className="queue-edit-checkbox queue-edit-checkbox--standalone">
                ...
                <div className="queue-edit-checkbox__content">
                  <label
                    htmlFor="distributeSubqueues"
                    className="queue-edit-checkbox__label"
                  >
                    Distribute tickets across subqueues
                  </label>
                  <p className="queue-edit-checkbox__hint">
                    Use defined percentages to divide tickets into as many as 5
                    subqueues. Each subqueue has its own primary and secondary
                    groups.
                  </p>
                </div>
              </div>
                  ...
                  <div className="queue-edit-form">
                    ...
                    <div className="queue-edit-checkbox queue-edit-checkbox--inline">
                      <button
                        ...
                        onClick={() =>
                          setEnableSecondaryGroups(!enableSecondaryGroups)
                        }
                      >
                        {enableSecondaryGroups ? (
                          <img
                            src={assetUrl("/assets/Check square.svg")}
                            alt=""
                            width="20"
                            height="20"
                          />
